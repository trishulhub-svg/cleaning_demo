...
export function ProfileClient({ user }: { user: CustomerData }) {
  ...
  const [profileState, profileAction, profilePending] = React.useActionState<FormState | null, FormData>(
    async (prev, formData) => {
      ...
      return updateCustomerProfile(prev, formData)
    },
    null
  )
  ...
  const [passwordState, passwordAction, passwordPending] = React.useActionState<FormState | null, FormData>(
    async (prev, formData) => {
      const { changeCustomerPassword } = await import('./actions')
      return changeCustomerPassword(prev, formData)
    },
    null
  )
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-sm bg-white">
        <CardHeader>
          ...
          <CardDescription>Update your name, phone, and address</CardDescription>
        </CardHeader>
        <CardContent>
          <form action={profileAction} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="name" name="name" defaultValue={user.name} required disabled={profilePending} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="phone">Phone Number</Label>
                <Input id="phone" name="phone" defaultValue={user.phone} disabled={profilePending} />
              </div>
            </div>

            <div className="space-y-2">
              <Label htmlFor="address">Address</Label>
              <Input id="address" name="address" defaultValue={user.address} placeholder="Enter your address" disabled={profilePending} />
            </div>
